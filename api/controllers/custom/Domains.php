 ...
 *
 */
class Domains_controller extends Common_api_functions  {
	...
	public function OPTIONS () {
		...
		$result['methods'] = array(
								array("href"=>"/api/".$this->_params->app_id."/Domains/", 	"methods"=>array(array("rel"=>"options", "method"=>"OPTIONS"))),
								array("href"=>"/api/".$this->_params->app_id."/Domains/{id}/","methods"=>array(array("rel"=>"read", 	"method"=>"GET"))),
							);
		# result
	...
	public function GET () {
		...
		if (!isset($this->_params->id) || $this->_params->id == "all") {
			$result = $this->Tools->fetch_all_objects ("Domains");
			if ($result===false)
			{ 
			...
			{ 
				return array("code"=>200, "data"=>$this->prepare_result($result, "Domains", true, true)); 
			}
		} elseif ( isset($this->_params->id) ) {
			$result =  $this->Tools->fetch_object ("Domains", "id", $this->_params->id);
			if ($result===false)
			{ 
				$this->Response->throw_exception(500, "Unable to read addresses"); 
			}
